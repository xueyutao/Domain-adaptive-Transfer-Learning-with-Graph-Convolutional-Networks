...
class DomainData(InMemoryDataset):
    ...
    def process(self):
        ...
        val_size = int(len(random_node_indices) * 0)
        train_node_indices = random_node_indices[:training_size]
        val_node_indices = random_node_indices[training_size:training_size + val_size]
        test_node_indices = random_node_indices[training_size + val_size:]
...
class DomainData2(InMemoryDataset):
    def __init__(self,
                 root,
                 name,
                 transform=None,
        ...
        self.name = name
        #self.root = root
        super(DomainData2, self).__init__(root, transform, pre_transform, pre_filter)
    ...
    def process(self):
        ...
        random_node_indices = np.random.permutation(y.shape[0])
        training_size = int(len(random_node_indices) * 0.6)
        # training_size2 = int(len(random_node_indices) * 0.8)
        val_size = int(len(random_node_indices) * 0)
        train_node_indices = random_node_indices[:training_size]
        val_node_indices = random_node_indices[training_size:training_size + val_size]
        test_node_indices = random_node_indices[training_size + val_size:]
